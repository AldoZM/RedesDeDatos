...
\begin{tikzpicture}[
    ...
    lbls/.style={font=\scriptsize\itshape, text=gray!70!black}
]

%% ── Lifelines ───────────────────────────────────────────────────────────────
\node[font=\bfseries\small, align=center] (sup) at (0, 0)
    {Supplicant\\{\scriptsize\itshape(Laptop)}};
\node[font=\bfseries\small, align=center] (aut) at (6, 0)
    {Authenticator\\{\scriptsize\itshape(Switch / AP)}};
\node[font=\bfseries\small, align=center] (rad) at (12, 0)
    {RADIUS Server\\{\scriptsize\itshape(FreeRADIUS/ISE)}};

%% Lifeline verticals
\draw[thick, gray!50] (0,  -0.4) -- (0,  -13.5);
\draw[thick, gray!50] (6,  -0.4) -- (6,  -13.5);
\draw[thick, gray!50] (12, -0.4) -- (12, -13.5);

%% Activation boxes
\draw[thick, fill=blue!10] (-0.25,-0.4) rectangle (0.25,-13.5);
\draw[thick, fill=orange!10] (5.75,-0.4) rectangle (6.25,-13.5);
\draw[thick, fill=violet!10] (11.75,-0.4) rectangle (12.25,-13.5);

%% State label: port unauthorized
\node[lbls, align=center] at (6, -1.1) {Puerto:\\UNAUTHORIZED};

%% ── Paso 1: EAPOL-Start ─────────────────────────────────────────────────────
\draw[msg, blue!70!black] (0.25,-1.6) -- (5.75,-1.6)
    node[midway, above, font=\small] {\textbf{EAPOL-Start}};
\node[lbls, left] at (0,-1.6) {(1)};

%% ── Paso 2: EAP-Request Identity ────────────────────────────────────────────
\draw[msgr, orange!70!black] (0.25,-2.6) -- (5.75,-2.6)
    node[midway, above, font=\small] {\textbf{EAP-Request / Identity}};
\node[lbls, left] at (0,-2.6) {(2)};

%% ── Paso 3: EAP-Response Identity ───────────────────────────────────────────
\draw[msg, blue!70!black] (0.25,-3.6) -- (5.75,-3.6)
    node[midway, above, font=\small] {\textbf{EAP-Response / Identity}};
\node[lbls, left] at (0,-3.6) {(3)};
\node[lbl, below right, font=\scriptsize] at (3,-3.6) {user@empresa.com};

%% ── Paso 4: RADIUS Access-Request ───────────────────────────────────────────
\draw[msg, orange!70!black] (6.25,-4.5) -- (11.75,-4.5)
    node[midway, above, font=\small] {\textbf{RADIUS Access-Request}};
\node[lbls, right] at (12,-4.5) {(4)};
\node[lbl, below right, font=\scriptsize] at (7,-4.5) {EAP-Message encapsulado};

%% ── Paso 5: EAP TLS Tunnel negotiation ──────────────────────────────────────
\draw[msgr, violet!70!black] (6.25,-5.5) -- (11.75,-5.5)
    node[midway, above, font=\small] {\textbf{Access-Challenge (TLS ServerHello)}};
\node[lbls, right] at (12,-5.5) {(5)};

\draw[msgr, orange!70!black] (0.25,-6.3) -- (5.75,-6.3)
    node[midway, above, font=\small] {\textbf{EAP-Request (TLS ServerHello)}};

\draw[msg, blue!70!black] (0.25,-7.1) -- (5.75,-7.1)
    node[midway, above, font=\small] {\textbf{EAP-Response (ClientCert + Finished)}};

\draw[msg, orange!70!black] (6.25,-7.9) -- (11.75,-7.9)
    node[midway, above, font=\small] {\textbf{Access-Request (ClientCert)}};

\node[lbls] at (6,-8.6) {\emoji{locked} Túnel TLS establecido};
\node[lbls] at (6,-9.0) {RADIUS verifica certificado X.509 del cliente};

%% ── Paso 6: RADIUS Access-Accept ────────────────────────────────────────────
\draw[msgr, green!60!black, line width=1.5pt] (6.25,-9.7) -- (11.75,-9.7)
    node[midway, above, font=\small\bfseries] {\textbf{RADIUS Access-Accept}};
\node[lbls, right] at (12,-9.7) {(6)};
\node[lbl, below right, font=\scriptsize, text=green!50!black] at (7,-9.7)
    {VLAN=10, Session-Timeout=3600};

%% ── Paso 7: EAP-Success ─────────────────────────────────────────────────────
\draw[msgr, green!60!black, line width=1.5pt] (0.25,-10.6) -- (5.75,-10.6)
    node[midway, above, font=\small\bfseries] {\textbf{EAP-Success}};
\node[lbls, left] at (0,-10.6) {(7)};

%% ── Paso 8: Puerto autorizado ───────────────────────────────────────────────
\node[lbls, align=center] at (6,-11.4) {Puerto:\\AUTHORIZED};
\node[font=\small, text=green!60!black, align=center] at (6,-12.3)
    {\textbf{Cliente asignado a VLAN 10}\\
     \textbf{Tráfico de datos permitido}};

%% Caption note
\node[font=\scriptsize\itshape, text=gray, align=center] at (6,-13.3)
