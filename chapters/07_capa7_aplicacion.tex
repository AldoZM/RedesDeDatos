...
\begin{tikzpicture}[
    ...
    server/.style={font=\bfseries\small}
]
\node[server] at (0,0) {Cliente};
\node[server] at (8,0) {Servidor DHCP};
\draw[thick] (0,-0.3) -- (0,-7.5);
\draw[thick] (8,-0.3) -- (8,-7.5);

\draw[msg, blue] (0,-1.0) -- (8,-1.8)
    node[midway, above, sloped, font=\small\bfseries] {DISCOVER};
\node[font=\tiny, right] at (8,-1.8)
    {Broadcast: ``¿Hay un servidor DHCP?''};

\draw[msg, green!60!black] (8,-2.3) -- (0,-3.1)
    node[midway, above, sloped, font=\small\bfseries] {OFFER};
\node[font=\tiny, left] at (0,-3.1)
    {``Te ofrezco 192.168.1.100 por 24h''};

\draw[msg, orange] (0,-3.6) -- (8,-4.4)
    node[midway, above, sloped, font=\small\bfseries] {REQUEST};
\node[font=\tiny, right] at (8,-4.4)
    {Broadcast: ``Acepto la oferta de 192.168.1.100''};

\draw[msg, red!70!black] (8,-4.9) -- (0,-5.7)
    node[midway, above, sloped, font=\small\bfseries] {ACK};
\node[font=\tiny, left] at (0,-5.7)
    {``Confirmado, es tuya hasta las 14:00 de mañana''};

\node[font=\small\itshape] at (4,-6.8)
